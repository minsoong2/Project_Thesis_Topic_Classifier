...
# 새로운 파일 생성 후 내용 쓰기
with open("new_x_labels_with_y.txt", 'w') as new_file:
    new_file.write(file_content)

# 텍스트 파일에서 국가과학기술표준분류체계와 개수 가져오기
with open('new_x_labels_with_y.txt', 'r') as file:
...
for i in range(count):
    ...
    for Y_, RT_, JN_, A_F, SC_1, SC_2, SC_3, code_count in zip(Year, ResultTitle, JournalName, Abstract, ScienceClass1, ScienceClass2, ScienceClass3, code_counts):
        ...
        if str(SC_1.get('code')) in paper_count and str(SC_2.get('code')) in paper_count and str(SC_3.get('code')) in paper_count:
            ...
        else:
            continue
...
print(paper_count)
save_dictionary(paper_count, "new_x_labels_with_y.txt")
f.close()
