...
# - 수정
collection = db['my_collection_selected_data_4']
...
with open("new_x_labels_with_y_4.txt", 'w') as new_file:
    new_file.write(file_content)

# 텍스트 파일에서 국가과학기술표준분류체계와 개수 가져오기 - 수정
with open('new_x_labels_with_y_4.txt', 'r') as file:
...
f = open(f"all.txt", "w", encoding='utf-8')
for i in range(count):
    ...
    for Y_, RT_, JN_, A_F, SC_1, SC_2, SC_3, code_count in zip(Year, ResultTitle, JournalName, Abstract, ScienceClass1, ScienceClass2, ScienceClass3, code_counts):
        ...
        num = -30
        # 국가과학기술분류코드 논문당 3개
        if paper_count[SC_1.get('code')] < num and paper_count[SC_2.get('code')] < num and paper_count[SC_3.get('code')] < num:
            continue
...
print(paper_count)
# 수정
save_dictionary(paper_count, "new_x_labels_with_y_4.txt")
f.close()
